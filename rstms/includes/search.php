...
$search_sql = $conn->prepare("SELECT * FROM tbl_food WHERE LOWER(f_title) LIKE ? OR LOWER(f_desc) LIKE ? OR CAST(f_disctprice AS CHAR) LIKE ?");
$searchParam = "%".strtolower($search)."%";
$search_sql -> bind_param("sss", $searchParam, $searchParam, $searchParam);
$search_sql -> execute();
...
if ($search_result && $search_result->num_rows>0) {
	...
	while ($row = $search_result->fetch_assoc()) {
        ...
        if ($count % 2 !== 0 || $count === $search_result->num_rows - 1) {
            echo "</div>";
        }
        $count++;
	}
